...
- If a role is assumed by something that is allowed to assume it, temporary security credentials are created.
    ...
    - If you want to access a resource in a different AWS account you can use an IAM Role or a resource-based policy
...
### For applications running outside of AWS, such as on-premises machines, AWS provides several solutions to securely access AWS resources and services. Here are some options:
- IAM Roles Anywhere: This service allows workloads running outside of AWS to obtain temporary AWS credentials using X.509 certificates. It's designed for servers, containers, and applications in on-premises, hybrid, or multi-cloud environments. Benefits include:
	- Eliminates the need to manage long-term credentials
 	- Uses the same IAM roles and policies as AWS workloads
  	- Provides a secure way to access AWS resources
- AWS Systems Manager for on-premises management: You can use Systems Manager to manage both your AWS and on-premises resources. This requires:
	- Installing the Systems Manager agent on your on-premises machines
 	- Configuring the necessary IAM permissions
  	- Registering your on-premises instances with Systems Manager
- VPN or AWS Direct Connect: For more comprehensive integration, you can set up a VPN connection or use AWS Direct Connect to establish a dedicated network connection between your on-premises infrastructure and AWS.
- AWS SDK and CLI with IAM users: For simpler scenarios, you can use AWS SDK or CLI on your on-premises machines with IAM user credentials. However, this approach requires careful management of access keys.
